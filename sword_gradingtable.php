 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sword_publish_table extends table_sql implements renderable {
    ...
    public function __construct(assign $assignment,
        ...
        parent::__construct('mod_sword_publish');
        ...
        $urlparams = array('action'=>'grading', 'id' => $cm_sword->id, 'assignment'=>$assignment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/sword/submissions.php', $urlparams);
        ...
        $fields .= 'ss.status as publish_status';
        ...
        $from = '{user} u
                         ...
                         LEFT JOIN {sword_submissions} ss ON ss.submission = s.id AND ss.type = \'assign\' AND ss.sword = :sword_id' ;

        $params["sword_id"]=$cm_sword->instance;
        ...
        $headers[] = get_string("publish_status","sword");
